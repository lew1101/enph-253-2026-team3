                                          ...
                                          const PoseSnapshot &pose_snapshot)
{
    ...
    _pose.path_speed_mps = pose_command.desired_speed_mps > 0.0f ? pose_command.desired_speed_mps
                                   ...
                                   float dt_s)
{
    ...
    // Progress the reference once per fixed-rate control iteration. If the
    // robot falls behind, hold the reference until it catches up.
    if (!_pose.path_endpoint_active) {
        ...
        if (reference_error_m <= POSE_PATH_REFERENCE_ADVANCE_TOLERANCE_M) {
            _pose.path_progress_m = std::min(_pose.path_progress_m + _pose.path_speed_mps * dt_s,
