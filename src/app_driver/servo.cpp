...
ServoDriver::ServoDriver(const Config &config) : _config(config) {}
...
esp_err_t ServoDriver::init() {
    ...
    if (_config.min_pulse_us >= _config.max_pulse_us)
        return ESP_ERR_INVALID_ARG;

    if (_config.freq_hz == 0)
        return ESP_ERR_INVALID_ARG;

    if (_config.min_pulse_deg >= _config.max_pulse_deg)
        return ESP_ERR_INVALID_ARG;

    if (_config.min_clamp_deg >= _config.max_clamp_deg)
        return ESP_ERR_INVALID_ARG;
    ...
    if (_config.max_pulse_us > (1000000UL / _config.freq_hz))
        return ESP_ERR_INVALID_ARG;
...
uint32_t ServoDriver::duty_levels() const {
    return 1UL << _config.duty_res;
}

uint32_t ServoDriver::max_duty() const {
    // range is [0, 2^duty_res - 1]
    return duty_levels() - 1;
}
