...
volatile MetalPhase s_phase = MetalPhase::PHASE_START_PULSE;
...
static void IRAM_ATTR on_metal_timer()
{
    ...
    switch (s_phase) {
        case MetalPhase::PHASE_START_PULSE:
            gpio_set_level(s_cfg.gpio_md_pulse, 1); // start pulse

            s_phase = MetalPhase::PHASE_END_PULSE;
            arm_timer_us(s_cfg.md_pulse_us); // pulse width
            break;

        case MetalPhase::PHASE_END_PULSE:
            gpio_set_level(s_cfg.gpio_md_pulse, 0); // end pulse

            s_phase = MetalPhase::PHASE_SAMPLE;
            arm_timer_us(s_cfg.md_blank_us); // blanking time before sampling
            break;

        case MetalPhase::PHASE_SAMPLE:
            if (s_task_handle != nullptr) {
                // sample time!
                vTaskNotifyGiveFromISR(s_task_handle, &higher_priority_task_woken);
            }

            s_phase = MetalPhase::PHASE_START_PULSE; // reset
            arm_timer_us(s_cfg.md_deadtime_us);      // deadtime before next pulse
            break;
    }

    if (higher_priority_task_woken == pdTRUE) {
        portYIELD_FROM_ISR(); // yield to higher priority task
    }
}
...
static esp_err_t timer_setup()
{
    ...
    timerAttachInterrupt(s_md_timer, &on_metal_timer);

    s_phase = MetalPhase::PHASE_START_PULSE;
                                    ...
                                    TaskHandle_t *out_handle)
{
    ...
    if (err != ESP_OK) {
        vTaskDelete(s_task_handle); // timer setup failed... delete task
