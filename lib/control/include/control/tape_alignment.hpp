 ...
 * 4. Record each sensor’s tape entry and exit positions
 * 5. Calculate the tape center
 * 6. Move back left to the calculated center
 */
...
struct TapeAlignmentConfig {
    ...
    float search_speed_mps = 0.10f;   // strafe speed during staging and scanning
    float staging_distance_m = 0.02f; // minimum distance before accepting that the robot is clear
    float max_staging_distance_m = 0.06f; // fail if it cannot get clear within this distance
    float max_scan_distance_m = 0.08f;    // fail if it doesn't cross tape within this distance
    ...
    uint8_t tape_alignment_debounce_samples = 3; // number of samples to confirm a tape edge
    uint32_t timeout_ms = 5000;                  // maximum duration of the entire procedure
};
...
struct TapeEdgeCapture {
    ...
    std::optional<float> tape_entry_position_m; //populatd when the sensor first sees tape
    std::optional<float> tape_exit_position_m;  // populated when last sees tape before leaving it
};
